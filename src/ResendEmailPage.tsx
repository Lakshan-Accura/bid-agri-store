...
const EmailSendPage: React.FC = () => {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/v1/user/resendVerifyToken?email=${encodeURIComponent(userEmail)}`,
        {
          method: 'GET',
          headers: {
  ...
  const handleBackToLogin = () => {
    navigate(getVerificationLoginRoute());
  };

  const handleCreateNewAccount = () => {
    clearRegistrationData();
    navigate(userType === 'store' ? '/store-signup' : '/farmer-signup');
  };
